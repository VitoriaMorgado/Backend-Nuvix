...
if(!empty($_SESSION["user"])) {
    foreach($_SESSION["user"] as $key => $user) {
        ...
        <tr>
            ...
            <td style="border:1px solid black"><img src="imagens/'.$user["userImage"].'" width="100"></td>
            <td style="border:1px solid black">'.$client["userName"].'</td>
            <td style="border:1px solid black">'.$client["userCPF"].'</td>
            <td style="border:1px solid black">'.$client["userEmail"].'</td>
            <td style="border:1px solid black">'.$client["userWhatsapp"].'</td>
        </tr>
        ';
    }
} else {
    echo '
    <tr>
        <td colspan="4">Nenhum user cadastrado</td>
    </tr>
    ';
}
...
<body>
    <h1 style="text-align:center">Lista de user</h1>
    <p style="text-align:center">Data: '.date('d/m/Y').'</p>
    <p style="text-align:center">Total de user: '.count($_SESSION["user"]).'</p>
    <table>
        <thead>
            <tr>
...
$nomeArquivo = 'user_'.date('YmdHis').'.pdf';
